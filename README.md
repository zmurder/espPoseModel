...
在 ESP32-S3 上实时检测坐姿:输入 OV3660 摄像头 320×240 图像,输出 6 个关键点(双眼/双耳/双肩),用于算距离/倾斜度判断坐姿是否端正;低头看不到眼时用双耳定位头部。
...
| 项 | 值 |
|---|---|
| 模型 | PoseNet,0.13M 参数,纯 heatmap(120×160),6 关键点(眼/耳/肩) |
| 算子 | ESP-DL int8:`Conv/Relu/Add/Resize/Sigmoid`(无 ConvTranspose,推理 10s→预期 150-300ms) |
| 量化 | int8 对称,`pose_model.espdl` ~150KB |
| PCK@0.1 (cam 真实域) | 96.50%(量化后,损失 1.15%);眼/耳近满分、肩膀~0.93 |
| 双肩倾斜角中位误差 | ~0.8°(坐姿判断阈值 10°,远够用) |
